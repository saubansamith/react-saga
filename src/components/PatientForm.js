...
const FORM_STORAGE_KEY = "medflow_patientFormDraft";
...
function PatientForm() {
    ...
    // Initialize state from local storage so it survives page reload
    const [formData, setFormData] = useState(() => {
        const saved = loadFromLocalStorage(FORM_STORAGE_KEY);
    ...
    const handleChange = (e) => {
        ...
        // Persist draft to local storage on every change
        saveToLocalStorage(FORM_STORAGE_KEY, newFormData);
    };
    ...
    const handleSubmit = (e) => {
        ...
        console.log("🚀 [Component] Dispatching SUBMIT_PATIENT_FORM:", payload);
        dispatch({ type: SUBMIT_PATIENT_FORM, payload });

        if (isOnline) {
            console.log("ℹ️ [Component] Form submitted online");
            showNotification("Form submitted successfully", "success");
        } else {
            console.log("ℹ️ [Component] Form queued offline");
        ...
        import("../utils/cryptoStorage").then(({ removeFromLocalStorage }) => {
